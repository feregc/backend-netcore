...
namespace SistemaVenta.BLL.Services
{
    public class ProductoService : IProductoService
    {
        ...
        public async Task<List<ProductoDTO>> Lista()
        {
            try
            {
                var queryProducto = await _productoRepositorio.Query();
        ...
        public async Task<ProductoDTO> Crear(ProductoDTO modelo)
        {
            try
            {
                var productoCreado = await _productoRepositorio.Create(_mapper.Map<Producto>(modelo));
        ...
        public async Task<bool> Editar(ProductoDTO modelo)
        {
            try
            {
                ...
                var productoEncontrado = await _productoRepositorio.Get(u =>
                ...
                bool respuesta = await _productoRepositorio.Update(productoEncontrado);
                if (!respuesta)
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                var productoEncontrado = await _productoRepositorio.Get(p => p.IdProducto == id);
                if (productoEncontrado == null)
                    throw new TaskCanceledException("El producto no existe");
